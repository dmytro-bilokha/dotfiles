...
menu_binds = {
    ...
    key({},          "о",       function (w) w.menu:move_down() end),
    key({},          "л",       function (w) w.menu:move_up()   end),
...
add_binds("normal", {
    ...
    -- Scrolling rus/ukr keyboard layout
    key({}, "о", "Scroll document down.",
        function (w) w:scroll{ yrel =  scroll_step } end),

    key({}, "л", "Scroll document up.",
        function (w) w:scroll{ yrel = -scroll_step } end),

    key({}, "р", "Scroll document left.",
        function (w) w:scroll{ xrel = -scroll_step } end),

    key({}, "д", "Scroll document right.",
        function (w) w:scroll{ xrel =  scroll_step } end),
 
    key({"Control"}, "у", "Scroll document down.",
        function (w) w:scroll{ yrel =  scroll_step } end),

    key({"Control"}, "н", "Scroll document up.",
        function (w) w:scroll{ yrel = -scroll_step } end),

    key({"Control"}, "в", "Scroll half page down.",
        function (w) w:scroll{ ypagerel =  0.5 } end),

    key({"Control"}, "г", "Scroll half page up.",
        function (w) w:scroll{ ypagerel = -0.5 } end),

    key({"Control"}, "а", "Scroll page down.",
        function (w) w:scroll{ ypagerel =  1.0 } end),

    key({"Control"}, "и", "Scroll page up.",
        function (w) w:scroll{ ypagerel = -1.0 } end),
    ...
    buf("^пп$", "Go to the top of the document.",
        function (w, b, m) w:scroll{ ypct = m.count } end, {count=0}),

    buf("^П$", "Go to the bottom of the document.",
        function (w, b, m) w:scroll{ ypct = m.count } end, {count=100}),
    ...
    key({}, "з", [[Open a URL based on the current primary selection contents
        in the current tab.]],
        function (w)
            local uri = luakit.selection.primary
            if not uri then w:notify("No primary selection...") return end
    ...
    key({}, "y", "Yank current URI to primary selection.",
     ...
     key({}, "н", "Yank current URI to primary selection.",
        function (w)
            local uri = string.gsub(w.view.uri or "", " ", "%%20")
            luakit.selection.primary = uri
            w:notify("Yanked uri: " .. uri)
    ...
    key({}, "W",
     ...
     -- Commands rus/ukr
    key({"Control"}, "ф", "Increment last number in URL.",
        function (w) w:navigate(w:inc_uri(1)) end),

    key({"Control"}, "ч", "Decrement last number in URL.",
        function (w) w:navigate(w:inc_uri(-1)) end),

    key({}, "щ", "Open one or more URLs.",
        function (w) w:enter_cmd(":open ") end),

    key({}, "е", "Open one or more URLs in a new tab.",
        function (w) w:enter_cmd(":tabopen ") end),

    key({}, "ц", "Open one or more URLs in a new window.",
        function (w) w:enter_cmd(":winopen ") end),

    key({}, "Щ", "Open one or more URLs based on current location.",
        function (w) w:enter_cmd(":open " .. (w.view.uri or "")) end),
    ...
    -- History rus/ukr
     key({}, "Р", "Go back in the browser history `[count=1]` items.",
        function (w, m) w:back(m.count) end),

    key({}, "Д", "Go forward in the browser history `[count=1]` times.",
        function (w, m) w:forward(m.count) end),

   key({"Control"}, "щ", "Go back in the browser history.",
        function (w, m) w:back(m.count) end),

    key({"Control"}, "ш", "Go forward in the browser history.",
        function (w, m) w:forward(m.count) end),
    ...
    -- Tab rus/ukr
    buf("^пЕ$", "Go to previous tab.",
        function (w) w:prev_tab() end),

    buf("^пе$", "Go to next tab (or `[count]` nth tab).",
        function (w, b, m)
            if not w:goto_tab(m.count) then w:next_tab() end
        end, {count=0}),

    buf("^п0$", "Go to first tab.",
        function (w) w:goto_tab(1) end),

    buf("^п$$", "Go to last tab.",
        function (w) w:goto_tab(-1) end),

    key({"Control"}, "е", "Open a new tab.",
        function (w) w:new_tab(globals.homepage) end),

    key({"Control"}, "ц", "Close current tab.",
        function (w) w:close_tab() end),

    key({}, "в", "Close current tab (or `[count]` tabs).",
        function (w, m) for i=1,m.count do w:close_tab() end end, {count=1}),

    key({}, "Б", "Reorder tab left `[count=1]` positions.",
        function (w, m)
            w.tabs:reorder(w.view, w.tabs:current() - m.count)
        end, {count=1}),

    key({}, "Ю", "Reorder tab right `[count=1]` positions.",
        function (w, m)
            w.tabs:reorder(w.view,
                (w.tabs:current() + m.count) % w.tabs:count())
        end, {count=1}),

    buf("^пР$", "Open homepage in new tab.",
        function (w) w:new_tab(globals.homepage) end),

    buf("^пр$", "Open homepage.",
        function (w) w:navigate(globals.homepage) end),

    buf("^пн$", "Duplicate current tab.",
        function (w) w:new_tab(w.view.history or "") end),

    key({}, "к", "Reload current tab.",
        function (w) w:reload() end),

    key({}, "К", "Reload current tab (skipping cache).",
        function (w) w:reload(true) end),

    key({"Control"}, "с", "Stop loading the current tab.",
        function (w) w.view:stop() end),

    key({"Control", "Shift"}, "К", "Restart luakit (reloading configs).",
        function (w) w:restart() end),
